    ...
    function esconderTooltips() {
      const elementos = Array.from(document.querySelectorAll('*')).filter(
        el => {
          ...
          const classTooltip = el.className
            ?.toString()
            .toLowerCase()
            .includes('tooltip');
          const idTooltip = el.id?.toLowerCase().includes('tooltip');
          const ariaTooltip = el
...
async function irParaPrimeiraPagina(page) {
  while (true) {
    const botaoAnterior = await page
      .locator('i.glyphicon.glyph-small.pbi-glyph-chevronleftmedium.middleIcon.active')
      .first();

    if (!(await botaoAnterior.isVisible())) break;

    // Verifica se está desabilitado
    const isDisabled = await botaoAnterior.getAttribute('disabled');
    const ariaDisabled = await botaoAnterior.getAttribute('aria-disabled');
...
export default async function capturarPaginasPowerBI() {
  ...
  const browser = await chromium.launch({
    ...
    slowMo: 10000, // Adiciona delay entre ações
  });
  ...
  try {
    ...
    await irParaPrimeiraPagina(page);
